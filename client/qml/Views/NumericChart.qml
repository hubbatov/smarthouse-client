...
Item{
	...
	Flickable {
		...
		Canvas {
            ...
            property double yStep: (height - bufferSize) / Math.abs(__private.maximum - __private.minimum)
			...
			onPaint: {
                ...
                var centerY = height /(__private.maximum - __private.minimum)*__private.maximum
                if(__private.minimum > 0) {
                    centerY = height - bufferSize / 2
                }
				...
				for(var i = 0; i < __data.count - 1; ++i){
                    ...
                    ctx.moveTo(__chartCanvas.yLabesWidth + i * xStep, centerY - value * yStep)
                    ctx.lineTo(__chartCanvas.yLabesWidth + (i + 1) * xStep, centerY - nextValue * yStep)
				}
			...
			MouseArea {
				...
				onPositionChanged: {
                    ...
                    var centerY = height /(__private.maximum - __private.minimum)*__private.maximum
                    if(__private.minimum > 0) {
                        centerY = height - __chartCanvas.bufferSize / 2
                    }
                    ...
                    var yy = centerY - value * __chartCanvas.yStep
